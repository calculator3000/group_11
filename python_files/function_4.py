...
def area_graph(country: str, normalize: bool):
    """
    Creates an area graph for a given country or the whole world. It can be specified if the graph 
    should show absolute numbers or absolute numbers, where the yearly output is always 100%.
    ...
    if country == "World" or country == None:
        country_df = data_df.groupby("Year", as_index=False).sum()
    ...
    # check if country input is in country list
        ...
        country_df = data_df[data_df["Entity"] == f"{country}"]
        if normalize is True:
            country_df["crop_output_quantity"] = (
                country_df["crop_output_quantity"] / country_df["output_quantity"]
            )
...
area_graph(""Angola"" , normalize = False)